...
const Header = () => {
  ...
  const handleClickMovilSub = (service: {
    name: string
    subservices: string[]
  }) => {
  ...
  return (
    <Grid container className={classes.root}>
      ...
      <Grid
        ...
        xs={7}
        lg={4}
        className={`${classes.container} ${classes.centerVertical}`}
      >
        <Typography
          variant='subtitle2'
          component='h2'
          className={classes.hiddenMovil}
        >
      ...
      >
        <List component='nav' aria-labelledby='nested-list-subheader'>
          {services.map(service => (
            <>
              <ListItemButton
                onClick={() => handleClickMovilSub(service)}
                key={service.name}
              >
                <ListItemText primary={service.name} />
                {service.subservices.length > 0 &&
                  (openMovilSub === service.name ? (
                    <ExpandLess />
                  ) : (
                    <ExpandMore />
                  ))}
                    ...
                    {service.subservices.map(subservice => (
                      <ListItemButton
                        ...
                        onClick={() =>
                          onChangeLink(
                            null,
                            `/services/${service.name}/${subservice}`
                          )
                        }
                      >
